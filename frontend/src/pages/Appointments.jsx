...
const Appointments = () => {
  ...
  useEffect(() => {
    ...
    const selectedHairdresser = hairdresser.find((h) => String(h._id) === String(styID));
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      let dayLabel = `${daysOfWeek[currentDate.getDay()]} ${currentDate.getDate()}/${currentDate.getMonth() + 1}`;
      newSlots.push({ day: dayLabel, date: new Date(currentDate), slots: dailySlots });
    }
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        <img className='sm:max-w-150 rounded-lg' src={hairdresserInfo.image} alt={hairdresserInfo.name} />
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 sm:max-w-200'>
          ...
          <p><strong>Specialitet:</strong> {hairdresserInfo.speciality}</p>
          <p><strong>Erfarenhet:</strong> {hairdresserInfo.experience}</p>
          <p><strong>Om:</strong> {hairdresserInfo.about}</p>
          <p><strong>Adress:</strong> {hairdresserInfo.address.line1}, {hairdresserInfo.address.line2}</p>
        </div>
      </div>
          ...
          {hairdresserSlots.map((dayItem, index) => (
            <button
              ...
              className={`px-3 py-2 text-sm rounded-lg font-semibold 
                ${selectedDay === dayItem.day ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black'}`}
                ...
                ?.slots.map((slot, index) => (
                  <button
                    ...
                    className={`text-sm px-2 py-1 rounded-lg text-center 
                      ${selectedTime === slot.start ? 'bg-blue-500 text-white' : 'bg-gray-100 border'}`}
